...
| Reference CachyOS patch snapshots | [`patches/cachyos/`](patches/cachyos) | Reference copies only. They are not auto-applied and the current disabled snapshots are not byte-identical to the active packaging patches. |
| Runtime tuning defaults | [`configs/MacPro6,1/sysctl.d/99-macpro.conf`](configs/MacPro6,1/sysctl.d/99-macpro.conf), [`packaging/arch/99-macpro.conf`](packaging/arch/99-macpro.conf), [`image/common/overlay/etc/sysctl.d/99-macpro.conf`](image/common/overlay/etc/sysctl.d/99-macpro.conf) | Same logical knobs live in three places. Keep them aligned; the package copy currently lacks the KVM `ignore_msrs` line present in the other two copies. |
...
- The active patch stack lives under [`packaging/arch/`](packaging/arch). The disabled snapshots under [`patches/cachyos/`](patches/cachyos) are archival/reference material and are currently not byte-identical to the active patch stack.
...
- The sysctl policy copies are not byte-identical: [`packaging/arch/99-macpro.conf`](packaging/arch/99-macpro.conf) lacks the `kvm.ignore_msrs` setting that exists in the model config and image overlay copies. Decide whether that is intentional before changing KVM docs or packaging defaults.
