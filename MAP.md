...
| Reference patch archive | [`patches/cachyos/`](patches/cachyos) | Disabled/reference copies of the CachyOS patch set. They are useful for comparison, but they are not the packaging source of truth; the current disabled snapshots are not byte-identical to the active stack. |
| Runtime tuning | [`configs/MacPro6,1/sysctl.d/99-macpro.conf`](configs/MacPro6,1/sysctl.d/99-macpro.conf), [`packaging/arch/99-macpro.conf`](packaging/arch/99-macpro.conf), [`image/common/overlay/etc/sysctl.d/99-macpro.conf`](image/common/overlay/etc/sysctl.d/99-macpro.conf), [`configs/MacPro6,1/fan/macfanctld.conf`](configs/MacPro6,1/fan/macfanctld.conf) | Sysctl defaults are duplicated for the config, package, and ISO paths, but the package copy currently lacks the `kvm.ignore_msrs` line present in the config and image copies. Fan behavior lives separately as an `applesmc`/`macfanctld` policy file. |
...
| [`packaging/arch/99-macpro.conf`](packaging/arch/99-macpro.conf) | Package-installed sysctl defaults. | Package-path copy of the tuning policy, currently missing the KVM `ignore_msrs` line that exists in the config and image overlay copies. |
...
| Path | Purpose | Notes |
|------|------|------|
| [`patches/cachyos/0001-bore.patch.disabled`](patches/cachyos/0001-bore.patch.disabled) | Reference copy of the BORE patch. | Not byte-identical to the active packaging patch. |
| [`patches/cachyos/0002-bbr3.patch.disabled`](patches/cachyos/0002-bbr3.patch.disabled) | Reference copy of the BBR3 patch. | Not byte-identical to the active packaging patch. |
| [`patches/cachyos/0003-cachy.patch.disabled`](patches/cachyos/0003-cachy.patch.disabled) | Reference copy of the main CachyOS tuning patch. | Not byte-identical to the active packaging patch. |
| [`patches/cachyos/0004-fixes.patch.disabled`](patches/cachyos/0004-fixes.patch.disabled) | Reference copy of the fixes patch. | Not byte-identical to the active packaging patch. |
...
- The active packaging patches and the disabled reference patches are related, but the current disabled snapshots are not byte-identical to the active stack.
- The three sysctl policy copies are not byte-identical today: the package-installed copy lacks `kvm.ignore_msrs`, while the model config and image overlay copies include it for macOS KVM guests.
